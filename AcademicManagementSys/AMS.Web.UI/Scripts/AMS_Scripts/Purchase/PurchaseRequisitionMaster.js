...
var PurchaseRequisitionMaster = {
    ...
    constructor: function () {
        ...
        $('#btnShowListforList').unbind('click').click(function () {
            ...
            if (valuPurchaseRequisitionType == " " || valuPurchaseRequisitionType == null) {
            ...
            else if (ValuTransDate == "" || ValuTransDate == null) {
                ...
                PurchaseRequisitionMaster.LoadList(valuPurchaseRequisitionType, ValuTransDate, StatusFlag, ReplishmentCode,AdminRoleID);
        ...
        $("#tblData tbody").on("keyup", "tr td input[id^=quantity]", function () {
            ...
            $(this).closest('tr').find('td').eq(12).text(parseFloat(parseFloat(taxamount)) + (parseFloat(totalamount)));
        ...
        $('#AddManual').on("click", function () {
            ...
            for (var i = 0; i < TotalRecord; i = i + 21) {
                ...
                if (DataArray1[i + 13] == DataArray[i + 11] && DataArray1[i + 4] == DataArray[i]) {
                    ...
                    '<td id="Location">' + DataArray[i + 11] + '</td>' +
                    ...
                    '<td  style="display:none">' +
                        ...
                        '<input type="hidden" id="LocAddress" value=' + DataArray[i + 11] + '>' +
        ...
        $("#CreatePurchaseRequisitionMasterClear").click(function () {
            ...
            $('select[name=STO]').val("");
            $('select[name=IssueFromLocationID]').val("");
            PurchaseRequisitionMaster.LoadDiv(selectedRequisitionType, selectedVendorID, RequsitionBy, CentreCode);
    ...
    TotalGrossAmmount: function () {
        ...
        $("#tblData tbody tr").each(function (i) {
            if (i < length) {
                x = parseFloat($(this).find('td').eq(12).text()).toFixed(2);
                a += parseFloat(x);
            }
        });
    ...
    TotalTaxAmounttt: function () {
        ...
        $("#tblDataEdit tbody tr").each(function (i) {
            if (i < length) {
                x = parseFloat($(this).find('td').eq(13).find('input[id^=ItemWiseTaxAmount]').val()).toFixed(2);
                a += parseFloat(x);
            }
        });
    ...
    TotalGrossAmmounttt: function () {
        ...
        $("#tblDataEdit tbody tr").each(function (i) {
            if (i < length) {
                x = parseFloat($(this).find('td').eq(12).find('input[id^=amt]').val()).toFixed(2);
                a += parseFloat(x);
            }
        });
    ...
    LoadList: function (PurchaseRequisitionType, TransDate, StatusFlag, AdminRoleID) {
        debugger
        ...
        $.ajax(
         {
             cache: false,
             type: "Get",
             data: { PurchaseRequisitionType: PurchaseRequisitionType, TransDate: TransDate, StatusFlag: 1, ReplishmentCode: ReplishmentCode, AdminRoleID: AdminRoleID },
             dataType: "html",
             url: '/PurchaseRequisitionMaster/List',
             success: function (data) {
                 //Rebind Grid Data
    ...
    LoadDiv: function (selectedRequisitionType, selectedVendorID, RequsitionBy, CentreCode) {
        $.ajax(
         {
             cache: false,
             type: "POST",
             data: { PurchaseRequisitionType: selectedRequisitionType, VendorID: selectedVendorID, PurchaseRequisitionBy: RequsitionBy, CentreCode: CentreCode },
             dataType: "html",
    ...
    GetXmlDataEditRequisation: function () {
        ...
        for (var i = 0; i < TotalRecord; i = i + 12)
        {
            if (DataArray[i + 1] == 0 || DataArray[i + 1] == "")
            {
               ...
               {
            ParameterXml = ParameterXml + "<row><ID>0</ID><Itemnumber>" + DataArray[i] + "</Itemnumber><Rate>" + DataArray[i + 2] + "</Rate><Quantity>" + DataArray[i + 1] + "</Quantity></row>";
               }
        }
