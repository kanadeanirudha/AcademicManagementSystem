...
* @sURL             String      Server side page tat will be notified
that order is changed
...
(function ($) {
    ...
    $.fn.rowReordering = function (options) {
        ...
        function _fnEndProcessingMode(oTable) {
            ///<summary>
            ///Function that ends the "Processing" mode and returns
            // the table in the normal state(Default function)
            ///</summary>
        ...
        function fnGetStartPosition(oTable, sSelector) {
            ...
            $(sSelector, oTable).each(function () {
                iPosition = parseInt(oTable.fnGetData(this,
properties.iIndexColumn));
        ...
        function fnCancelSorting(oTable, tbody, properties, iLogLevel,
sMessage) {
        ...
        function fnGetState(oTable, sSelector, id) {
            ...
            var tr = $("#" + id, oTable);
            var iCurrentPosition = oTable.fnGetData(tr[0],
properties.iIndexColumn);
            ...
            if (trPrevious.length > 0) {
                iNewPosition =
parseInt(oTable.fnGetData(trPrevious[0], properties.iIndexColumn));
                ...
                if (trNext.length > 0) {
                    iNewPosition =
parseInt(oTable.fnGetData(trNext[0], properties.iIndexColumn));
            ...
            return {
                sDirection: sDirection, iCurrentPosition:
    iCurrentPosition, iNewPosition: iNewPosition
            };

        }

        function fnMoveRows(oTable, sSelector, iCurrentPosition,
iNewPosition, sDirection, id, sGroup) {
            ...
            $(oTable.fnGetNodes()).each(function () {
                if (sGroup != "" && $(this).attr("data-group") != sGroup)
                    return;
                var tr = this;
                var iRowPosition = parseInt(oTable.fnGetData(tr,
properties.iIndexColumn));
            ...
            $("tbody", oTable).disableSelection().sortable({
                ...
                update: function (event, ui) {
                    ...
                    if (properties.bGroupingUsed) {
                        sGroup =
$(ui.item).attr(properties.sDataGroupAttribute);
                        if (sGroup == null || sGroup == undefined) {
                            fnCancelSorting($dataTable, tbody,
properties, 3, "Grouping row cannot be moved");
                            return;
                        }
                        sSelector = "tbody tr[" +
properties.sDataGroupAttribute + " ='" + sGroup + "']";
                    }

                    var oState = fnGetState($dataTable, sSelector,
ui.item.context.id);
                    ...
                    if (properties.sURL != null) {
                        ...
                        var oAjaxRequest = {
                            ...
                            success: function (data) {
                                ...
                                fnMoveRows($dataTable, sSelector,
oState.iCurrentPosition, oState.iNewPosition, oState.sDirection,
ui.item.context.id, sGroup);
                                properties.fnEndProcessingMode($dataTable);
                            },
                            error: function (jqXHR) {
                                fnCancelSorting($dataTable, tbody,
properties, 1, jqXHR.statusText);
                            }
                        };
                        properties.fnUpdateAjaxRequest(oAjaxRequest,
