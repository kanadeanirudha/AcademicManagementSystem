        ...
        public ActionResult Index()
        {
            try
            {
                if (Convert.ToInt32(Session["SuperUser"]) > 0 || Convert.ToInt32(Session["FinMgr"]) > 0)
                {
                    ...
                    return View("/Views/Accounts/AccountIndividualBalanceReport/Index.cshtml", model);
                }
                else
                {
        ...
        public List<AccountGeneralLedgerReport> GetAccountDetailsForReport()
        {
            try
            {
                ...
                if (_balanesheetMstID > 0 && _sessionFromDate != string.Empty && _sessionUptoDate != string.Empty && _accountId != 0 && _accSessionId != 0 && _personType != string.Empty)
                {
                    ...
                    IBaseEntityCollectionResponse<AccountGeneralLedgerReport> baseEntityCollectionResponse = _accountGeneralLedgerReportServiceAccessServiceAccess.GetByIndividualBalanceReportSearch(searchRequest);
                    if (baseEntityCollectionResponse != null)
                    {
                        if (baseEntityCollectionResponse.CollectionResponse != null && baseEntityCollectionResponse.CollectionResponse.Count > 0)
                        {
