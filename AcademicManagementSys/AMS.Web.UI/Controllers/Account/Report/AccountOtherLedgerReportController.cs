    ...
    public class AccountOtherLedgerReportController : BaseController
    {
        ...
        public ActionResult Index()
        {
            try
            {
                if (Convert.ToInt32(Session["SuperUser"]) > 0 || Convert.ToInt32(Session["FinMgr"]) > 0)
                {
                    ...
                    return View("/Views/Accounts/AccountOtherLedgerReport/Index.cshtml", model);
                }
                else
                {
        ...
        protected List<AccountGeneralLedgerReport> GetPersonNameListByPersonTypeAndAccountId(int AccountID, int AccBalsheetMstID, string PersonType)
        {
            try
            {
                ...
                IBaseEntityCollectionResponse<AccountGeneralLedgerReport> baseEntityCollectionResponse = _accountGeneralLedgerReportServiceAccess.GetPersonNameListByPersonTypeAndAccountId(searchRequest);
        ...
        public List<AccountGeneralLedgerReport> GetAccountOtherLedgerReportData(string centreCode)
        {
            try
            {
                ...
                if (_balanesheetMstID > 0 && _accountID > 0 && _personID > 0 && centreCode != string.Empty)
                {
                    ...
                    IBaseEntityCollectionResponse<AccountGeneralLedgerReport> baseEntityCollectionResponse = _accountGeneralLedgerReportServiceAccess.GetOtherLedgerBySearch(searchRequest);
                    if (baseEntityCollectionResponse != null)
                    {
                        if (baseEntityCollectionResponse.CollectionResponse != null && baseEntityCollectionResponse.CollectionResponse.Count > 0)
                        {
