...
namespace AMS.Web.UI.Controllers
{
    ...
    {
        ILeaveAvailedServiceAccess _LeaveAvailedServiceAccess = null;
        ...
        {
            _LeaveAvailedServiceAccess = new LeaveAvailedServiceAccess();
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (model != null && model.LeaveAvailedDTO != null)
                    {
                        ...
                        IBaseEntityResponse<LeaveAvailed> response = _LeaveAvailedServiceAccess.InsertLeaveAvailed(model.LeaveAvailedDTO);
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (model != null && model.LeaveAvailedDTO != null)
                    {
                        ...
                        model.LeaveAvailedDTO.CreatedBy = Convert.ToInt32(Session["UserID"]);
                        IBaseEntityResponse<LeaveAvailed> response = _LeaveAvailedServiceAccess.InsertLeaveAvailed(model.LeaveAvailedDTO);
                        model.LeaveAvailedDTO.errorMessage = CheckErrorV2((response.Entity != null) ? response.Entity.ErrorCode : 20, ActionModeEnum.Insert);
                    }
                    return Json(model.LeaveAvailedDTO.errorMessage, JsonRequestBehavior.AllowGet);
       ...
       //  Non-Action Methods
        ...
        public IEnumerable<LeaveAvailedViewModel> GetLeaveAvailedRecords(out int TotalRecords, string PersonID, string TaskNotificationMasterID)
        {
            ...
            IBaseEntityCollectionResponse<LeaveAvailed> baseEntityCollectionResponse = _LeaveAvailedServiceAccess.GetLeaveRequestForApproval_ByPersonID(searchRequest);
