...
namespace AMS.Web.UI.Controllers
{
    public class AccountCentreOpeningBalanceController : BaseController
    {
        ...
        IAccountCentreOpeningBalanceServiceAccess _accountCentreOpeningBalanceServiceAccess = null;
        ...
        {
            _accountCentreOpeningBalanceServiceAccess = new AccountCentreOpeningBalanceServiceAccess();
        ...
        public ActionResult Create(AccountCentreOpeningBalanceViewModel model)
        {
            try
            {
                if (model != null && model.AccountCentreOpeningBalanceDTO != null)
                {
                    ...
                    IBaseEntityResponse<AccountCentreOpeningBalance> response = _accountCentreOpeningBalanceServiceAccess.InsertAccountCentreOpeningBalance(model.AccountCentreOpeningBalanceDTO);
                    model.AccountCentreOpeningBalanceDTO.errorMessage = CheckErrorV2((response.Entity != null) ? response.Entity.ErrorCode : 20, ActionModeEnum.Update);
        ...
        public ActionResult IndividualOpeningBalance(AccountCentreOpeningBalanceViewModel model)
        {
            try
            {
                if (model != null && model.AccountCentreOpeningBalanceDTO != null)
                {
                    ...
                    model.AccountCentreOpeningBalanceDTO.CreatedBy = Convert.ToInt32(Session["UserID"]);
                    IBaseEntityResponse<AccountCentreOpeningBalance> response = _accountCentreOpeningBalanceServiceAccess.UpdateAccountCentreOpeningBalance(model.AccountCentreOpeningBalanceDTO);
                    model.AccountCentreOpeningBalanceDTO.errorMessage = CheckErrorV2((response.Entity != null) ? response.Entity.ErrorCode : 20, ActionModeEnum.Update);
                    return Json(model.AccountCentreOpeningBalanceDTO.errorMessage, JsonRequestBehavior.AllowGet);
                }
                else
        ...
        public List<AccountCentreOpeningBalance> GetListAccountCentreOpeningBalance(string SelectedBalanceSheet, string SelectedAccountType, string SelectedSessionID, out int TotalRecords)
        {
            ...
            IBaseEntityCollectionResponse<AccountCentreOpeningBalance> baseEntityCollectionResponse = _accountCentreOpeningBalanceServiceAccess.GetBySearch(searchRequest);
        ...
        public List<AccountCentreOpeningBalance> GetListAccountIndividualOpeningBalance(int SelectedBalanceSheet, int SelectedAccountID, string SelectedPersonType, string SelectedSessionID, out int TotalRecords)
        {
            ...
            searchRequest.SearchBy = _searchBy;
            searchRequest.SortDirection = _sortDirection;
            IBaseEntityCollectionResponse<AccountCentreOpeningBalance> baseEntityCollectionResponse = _accountCentreOpeningBalanceServiceAccess.GetBySearchIndividualAccount(searchRequest);
            if (baseEntityCollectionResponse != null)
            {
                if (baseEntityCollectionResponse.CollectionResponse != null && baseEntityCollectionResponse.CollectionResponse.Count > 0)
                {
        ...
        public ActionResult AjaxHandlerIndividualOpeningBalance(JQueryDataTableParamModel param, string AccBalsheetMstID, string AccountID, string PersonType, string SelectedSessionID)
        {
            ...
            if (PersonType == "E")
            {
                ...
                {
                    _searchBy = "EmployeeName Like '%" + param.sSearch + "%'  or EmployeeMiddleName Like '%" + param.sSearch + "%'  or EmployeeLastName Like '%" + param.sSearch + "%'";        //this "if" block is added for search functionality
                }
            }
            if (PersonType == "S")
            {
