...
namespace AMS.Web.UI.Controllers
{
    ...
    {
        IAdminRoleModuleAccessServiceAccess _adminRoleModuleAccessServiceAcess = null;
        IAdminSnPostsServiceAccess _adminSnPostsServiceAcess = null;
        ...
        IAdminRoleMasterServiceAccess _adminRoleMasterServiceAcess = null;
        ...
        {
            _adminRoleModuleAccessServiceAcess = new AdminRoleModuleAccessServiceAccess();
            ...
            _adminRoleMasterServiceAcess = new AdminRoleMasterServiceAccess();
            _adminSnPostsServiceAcess = new AdminSnPostsServiceAccess();
        }
        ...
        {
            if (Convert.ToString(Session["UserType"]) == "A" || Convert.ToInt32(Session["SuperUser"]) > 0 || Convert.ToInt32(Session["AcadMgr"]) > 0 || Convert.ToInt32(Session["EstMgr"]) > 0 || Convert.ToInt32(Session["AdmMgr"]) > 0)
            {
                return View();
            }
        ...
        public ActionResult List(string centerCode, string departmentID, string actionMode)
        {
            try
            {
                ...
                return PartialView("List", _adminRoleModuleAccessBaseViewModel);
        ...
        public ActionResult Create(string AdminRoleMasterID, AdminRoleModuleAccessViewModel model)
        {
            try
            {
                ...
                IBaseEntityResponse<AdminRoleModuleAccess> response = _adminRoleModuleAccessServiceAcess.SelectByAdminRoleMasterID(model.AdminRoleModuleAccessDTO);
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (model != null && model.AdminRoleModuleAccessDTO != null)
                    {
                        ...
                        IBaseEntityResponse<AdminRoleModuleAccess> response = _adminRoleModuleAccessServiceAcess.InsertAdminRoleModuleAccess(model.AdminRoleModuleAccessDTO);
                        model.AdminRoleModuleAccessDTO.errorMessage = CheckErrorV2((response.Entity != null) ? response.Entity.ErrorCode : 20, ActionModeEnum.Insert);
                    }
        ...
        public IEnumerable<AdminRoleModuleAccessViewModel> GetAdminRoleModuleAccess(string centerCode, int departmentId, out int TotalRecords)
        {
            ...
            IBaseEntityCollectionResponse<AdminRoleModuleAccess> baseEntityCollectionResponse = _adminRoleModuleAccessServiceAcess.GetBySearch(searchRequest);
        ...
        protected List<AdminRoleModuleAccess> GetAccessibleCentreList(int ID)
        {
            try
            {
                ...
                IBaseEntityCollectionResponse<AdminRoleModuleAccess> baseEntityCollectionResponse = _adminRoleModuleAccessServiceAcess.GetAccessibleCentreListByID(searchRequest);
                if (baseEntityCollectionResponse != null)
                {
                    if (baseEntityCollectionResponse.CollectionResponse != null && baseEntityCollectionResponse.CollectionResponse.Count > 0)
                    {
        ...
        protected List<AdminRoleModuleAccess> GetEntity(string centreCode, int adminRoleMasterID, string MonitoringLevel, string entityType)
        {
            try
            {
                ...
                IBaseEntityCollectionResponse<AdminRoleModuleAccess> baseEntityCollectionResponse = _adminRoleModuleAccessServiceAcess.GetEntityByID(searchRequest);
                if (baseEntityCollectionResponse != null)
                {
                    if (baseEntityCollectionResponse.CollectionResponse != null && baseEntityCollectionResponse.CollectionResponse.Count > 0)
                    {
        ...
        public IEnumerable<AdminRoleModuleAccessViewModel> GetAdminEntityIndividualList(string AccessibleCentreCode, string MonitoringLevel, string EntityType, int ID)
        {
            try
            {
                ...
                List<AdminRoleModuleAccess> listAdminRoleModuleAccess = new List<AdminRoleModuleAccess>();
                IBaseEntityCollectionResponse<AdminRoleModuleAccess> baseEntityCollectionResponse = _adminRoleModuleAccessServiceAcess.GetAdminEntityInduvidualListBySearch(searchRequest);
                if (baseEntityCollectionResponse != null)
                {
                    if (baseEntityCollectionResponse.CollectionResponse != null && baseEntityCollectionResponse.CollectionResponse.Count > 0)
                    {
        ...
        public ActionResult AjaxHandlerForCreate(JQueryDataTableParamModel param, string AccessibleCentreCode, string MonitoringLevel, string EntityType, string AdminRoleMasterID)
        {
            ...
            var result = from c in displayedFields select new[] { c.status.ToString(), c.DepartmentName.ToString(), c.Section.ToString(), c.CourseYear.ToString(), c.balSheetName.ToString(), c.StoreName.ToString(), c.IsAcadMgr.ToString(), c.IsEstMgr.ToString(), c.IsFinMgr.ToString(), c.IsAdmMgr.ToString(), c.IsSuperUser.ToString(), c.Acad.ToString(), c.Est.ToString(), c.Fin.ToString(), c.Adm.ToString(), c.super.ToString(), Convert.ToString(c.DptBshtSecnStrID), Convert.ToString(c.AdminRoleDetailsID) };
            try
            {
                if (Session["entityType"].ToString() != "null")
                {
                    if (Session["entityType"].ToString() == "DEPARTMENT")
                    {
                        result = from c in displayedFields select new[] { c.status.ToString(), c.DepartmentName.ToString(), c.IsAcadMgr.ToString(), c.IsEstMgr.ToString(), c.AdminRoleDetailsID.ToString(), c.DptBshtSecnStrID.ToString(), c.IsSuperUser.ToString() };
                    }
                    if (Session["entityType"].ToString() == "STORE")
                    {
                        result = from c in displayedFields select new[] { c.status.ToString(), c.DepartmentName.ToString(), c.IsAdmMgr.ToString(), c.AdminRoleDetailsID.ToString(), c.DptBshtSecnStrID.ToString() };
                    }
                    if (Session["entityType"].ToString() == "BALSHEET")
                    {
                        result = from c in displayedFields select new[] { c.status.ToString(), c.balSheetName.ToString(), c.IsFinMgr.ToString(), c.AdminRoleDetailsID.ToString(), c.DptBshtSecnStrID.ToString() };
                    }
                    if (Session["entityType"].ToString() == "SECTION")
                    {
                        result = from c in displayedFields select new[] { c.status.ToString(), c.DepartmentName.ToString(), c.IsAcadMgr.ToString(), c.AdminRoleDetailsID.ToString(), c.DptBshtSecnStrID.ToString() };
                    }
                    if (Session["entityType"].ToString() == "COURSEYEAR")
                    {
                        result = from c in displayedFields select new[] { c.status.ToString(), c.CourseYear.ToString(), c.IsAcadMgr.ToString(), c.AdminRoleDetailsID.ToString(), c.DptBshtSecnStrID.ToString() };
                    }
                }
