    ...
    public class OnlineExamExaminationCourseApplicableController : BaseController
    {
        ...
        public ActionResult Delete(int ID)
        {
            ...
            if (ID > 0)
            {
                ...
                {
                    string[] arrayList = { "This Course can not be deleted. Schedule is created for selected Course.", "warning", string.Empty };
        ...
        public ActionResult AnnounceResult(int ID)
        {
            var errorMessage = string.Empty;
            if (ID > 0)
            {
                ...
                OnlineExamExaminationCourseApplicableDTO.ConnectionString = _connectioString;
                OnlineExamExaminationCourseApplicableDTO.ID = Convert.ToInt32(ID);
                OnlineExamExaminationCourseApplicableDTO.ModifiedBy = Convert.ToInt32(Session["UserID"]);
                response = _OnlineExamExaminationCourseApplicableServiceAcess.AnnounceResult(OnlineExamExaminationCourseApplicableDTO);
                errorMessage = CheckErrorV2((response.Entity != null) ? response.Entity.ErrorCode : 20, ActionModeEnum.Delete);
            }
            ...
            if (Session["UserType"] != null)
            {
                ...
                switch (Convert.ToInt32(sortColumnIndex))
                {
                    case 0:
                        ...
                        {
                            _searchBy = "B.Description Like '%" + param.sSearch + "%' or B.DepartmentName Like '%" + param.sSearch + "%' or B.ExaminationStartDate Like'%" + param.sSearch + "%' or B.ExaminationEndDate Like'%" + param.sSearch + "%' or B.ResultAnnounceDate Like'%" + param.sSearch + "%' or A.ExaminationName Like'%" + param.sSearch + "%'";
                        }
                        break;

                    case 1:
                        _sortBy = "A.ExaminationName" + sortDirection + " B.Description";
                        if (string.IsNullOrEmpty(param.sSearch))
                        {
                            _searchBy = string.Empty;
                        }
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        _sortBy = "B.ExaminationStartDate";
                        if (string.IsNullOrEmpty(param.sSearch))
                        {
                            _searchBy = string.Empty;
                        }
                ...
                var result = from c in records select new[] { Convert.ToString(c.Description), Convert.ToString(c.ExaminationName), Convert.ToString(c.DepartmentName), Convert.ToString(c.Semester), Convert.ToString(c.ExaminationStartDate), Convert.ToString(c.ExaminationEndDate), Convert.ToString(c.ResultAnnounceDate), Convert.ToString(c.ID), Convert.ToString(c.OnlineExamExaminationMasterId), Convert.ToString(c.IsResultAnnounce), Convert.ToString(c.IsAllChecked) };
