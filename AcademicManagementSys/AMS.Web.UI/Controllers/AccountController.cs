...
namespace AMS.Web.UI.Controllers
{
    public class AccountController : BaseController
    {
            ...
            try
            {
                if (Command == "Next")
                {
                    if (Convert.ToString(System.Configuration.ConfigurationManager.AppSettings["IsManualAttendance"]) == "0")
                    {
                        if (!string.IsNullOrEmpty(model.EmailID) && !string.IsNullOrEmpty(model.Latitude) && !string.IsNullOrEmpty(model.Longitude))
                        {
                            ...
                            IBaseEntityResponse<UserMaster> response = _userMasterServiceAcess.SelectByEmailID(_userMasterViewModel.UserMasterDTO);
                ...
                else if (Command == "Submit")
                {
                    ...
                    if (ModelState.IsValid && model != null && !string.IsNullOrEmpty(model.EmailID) && !string.IsNullOrEmpty(model.Password))
                    {
                        ...
                        IBaseEntityResponse<UserMaster> response = _userMasterServiceAcess.SelectByEmailIDPassword(_userMasterViewModel.UserMasterDTO);
                        if (response.Entity.ErrorMessage != "")
                        {
        ...
        public ActionResult LogOff(string latitude, string longitude)
        {
            ...
            IBaseEntityResponse<UserMaster> response = _userMasterServiceAcess.LogOffByUserID(_userMasterViewModel.UserMasterDTO);
        ...
        [HttpGet]
        [AllowAnonymous]
        public ActionResult LoginReset(string id)
        {
            try
            {
                UserMasterViewModel _userMasterViewModel = new UserMasterViewModel();
                ...
                var userID = Base64Decode(splitData[0]);

                byte[] data = Convert.FromBase64String(splitData[1]);
                DateTime when = DateTime.FromBinary(BitConverter.ToInt64(data, 0));
                if (when < DateTime.UtcNow.AddHours(-1))
                {
                    _userMasterViewModel.UserMasterDTO = new UserMaster();
                    _userMasterViewModel.UserMasterDTO.Status = 400;
                    _userMasterViewModel.UserMasterDTO.ErrorMessage = "The link is expired. Please try login again.";
                }
                else
                {
                    ...
                    {
                        _userMasterViewModel.UserMasterDTO = new UserMaster();
                        _userMasterViewModel.UserMasterDTO.UserID = Convert.ToInt32(userID);
                        _userMasterViewModel.UserMasterDTO.ConnectionString = _connectioString;
                        IBaseEntityResponse<UserMaster> response = _userMasterServiceAcess.UserLoginReset(_userMasterViewModel.UserMasterDTO);
