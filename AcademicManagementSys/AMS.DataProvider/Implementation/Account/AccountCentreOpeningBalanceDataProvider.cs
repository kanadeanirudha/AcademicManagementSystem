            ...
            try
            {
                ...
                else
                {
                    ...
                    while (sqlDataReader.Read())
                    {
                        ...
                        {
                             if (DBNull.Value.Equals(sqlDataReader["EmpolyeeID"]) == false)
                            {
                                item.PersonID = Convert.ToInt32(sqlDataReader["EmpolyeeID"]);
                            }
                            if (DBNull.Value.Equals(sqlDataReader["EmpolyeeName"]) == false)
                            {
                                item.PersonName = Convert.ToString(sqlDataReader["EmpolyeeName"]);
                            }
                        }
