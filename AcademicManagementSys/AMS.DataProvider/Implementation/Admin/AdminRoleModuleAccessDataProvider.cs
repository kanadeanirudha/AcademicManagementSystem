        ...
        public IBaseEntityResponse<AdminRoleModuleAccess> UpdateAdminRoleModuleAccess(AdminRoleModuleAccess item)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    cmdToExecute.Parameters.Add(new SqlParameter("@bIsSuperUser", SqlDbType.Bit, 1, ParameterDirection.Input, true, 0, 0, "", DataRowVersion.Proposed, item.IsSuperUser));
            ...
            try
            {
                ...
                else
                {
                    ...
                    while (sqlDataReader.Read())
                    {
                        ...
                        //_item.IsFinMgr = Convert.ToBoolean(sqlDataReader["IsFinMgr"]);
                        //_item.IsAdmMgr = Convert.ToBoolean(sqlDataReader["IsAdmMgr"]);
                        //_item.IsDefaultRole = Convert.ToBoolean(sqlDataReader["IsDefaultRole"]);
                        //_item.IsCopyForSame = Convert.ToBoolean(sqlDataReader["IsCopyForSame"]);
                ...
                else
                {
                    ...
                    while (sqlDataReader.Read())  
                    {
                       ...
                       // item.DptBshtSecnStrID = Convert.ToInt32(sqlDataReader["DptBshtSecnStrID"]);
                        ...
                        //{
                        //    item.DptBshtSecnStrID = Convert.ToInt32(sqlDataReader["DepartmentID"]);
                        //}

                        if (searchRequest.EntityType == "DEPARTMENT")
                        {
                            if (DBNull.Value.Equals(sqlDataReader["DepartmentID"]) == false)
                            {
                                item.DptBshtSecnStrID = Convert.ToInt32(sqlDataReader["DepartmentID"]);
                        ...
                        if (searchRequest.EntityType == "BALSHEET")
                        {
                            if (DBNull.Value.Equals(sqlDataReader["BalSheetID"]) == false)
                            {
                                item.DptBshtSecnStrID = Convert.ToInt32(sqlDataReader["BalSheetID"]);
                            }
                        }
                        if (searchRequest.EntityType == "COURSEYEAR")
                        {
                            if (DBNull.Value.Equals(sqlDataReader["OrgCentrwiseCourseYearID"]) == false)
                            {
                                item.DptBshtSecnStrID = Convert.ToInt32(sqlDataReader["OrgCentrwiseCourseYearID"]);
                               item.CourseYear = sqlDataReader["Description"].ToString();
                            }
                        }
                        ...
                        if (DBNull.Value.Equals(sqlDataReader["departmentName"]) == true)
                        {
                            item.DepartmentName = "-".ToString();
                        }

                        if (DBNull.Value.Equals(sqlDataReader["OrgCentrewiseDepartmentID"]) == false)
                        {
                            item.OrgCentrewiseDepartmentID = Convert.ToInt32(sqlDataReader["OrgCentrewiseDepartmentID"]);
                        }
                        if (DBNull.Value.Equals(sqlDataReader["OrgCentrewiseDepartmentID"]) == true)
                        {
                            item.OrgCentrewiseDepartmentID = 0;
                        }

                        if (DBNull.Value.Equals(sqlDataReader["BalSheetID"]) == false)
                        {
                            item.BalSheetID = Convert.ToInt32(sqlDataReader["BalSheetID"]);
                        }
                        if (DBNull.Value.Equals(sqlDataReader["BalSheetID"]) == true)
                        {
                            item.BalSheetID = 0;
                        }

                        if (DBNull.Value.Equals(sqlDataReader["section"]) == false)
                        {
                            item.Section = sqlDataReader["section"].ToString();
                        }
                        if (DBNull.Value.Equals(sqlDataReader["section"]) == true)
                        {
                            item.Section = "-".ToString();
                        }

                        //if (DBNull.Value.Equals(sqlDataReader["CourseYear"]) == false)
                        //{
                        //    item.CourseYear = sqlDataReader["CourseYear"].ToString();
                        //}
                        //if (DBNull.Value.Equals(sqlDataReader["CourseYear"]) == true)
                        //{
                        //    item.CourseYear = "-".ToString();
                        //}

                        if (DBNull.Value.Equals(sqlDataReader["balSheetName"]) == false)
                        {
                            item.balSheetName = sqlDataReader["balSheetName"].ToString();
                        }
                        if (DBNull.Value.Equals(sqlDataReader["balSheetName"]) == true)
                        {
                            item.balSheetName = "-".ToString();
                        }

                        if (DBNull.Value.Equals(sqlDataReader["StoreName"]) == false)
                        {
                            item.StoreName = sqlDataReader["StoreName"].ToString();
                        }
                        if (DBNull.Value.Equals(sqlDataReader["StoreName"]) == true)
                        {
                        ...
                        if (DBNull.Value.Equals(sqlDataReader["IsAcadMgr"]) == true)
                        {
                            item.IsAcadMgr = false;
                            item.Acad = "AcadFalse";
                        }
                        ...
                        if (DBNull.Value.Equals(sqlDataReader["IsEstMgr"]) == true)
                        {
                            item.IsEstMgr = false;
                            item.Est = "EstFalse";
                        }
                        ...
                        if (DBNull.Value.Equals(sqlDataReader["IsFinMgr"]) == true)
                        {
                            item.IsFinMgr = false;
                            item.Fin = "FinFalse";
                        }
                        ...
                        if (DBNull.Value.Equals(sqlDataReader["IsAdmMgr"]) == true)
                        {
                            item.IsAdmMgr = false;
                            item.Adm = "AdmFalse";
                        }
                        ...
                        if (DBNull.Value.Equals(sqlDataReader["IsSuperUser"]) == false)
                        {
                            ...
                            item.super = "superTrue";
                        }
                        if (DBNull.Value.Equals(sqlDataReader["IsSuperUser"]) == true)
                        {
                            item.IsSuperUser = false;
                            item.super = "superFalse";
                        }
