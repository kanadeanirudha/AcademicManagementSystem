        ...
        public IBaseEntityResponse<AdminRoleCentreRights> GetCentreLevelManagerRights(AdminRoleCentreRights item)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    cmdToExecute.CommandType = CommandType.StoredProcedure;
                    cmdToExecute.Parameters.Add(new SqlParameter("@iAdminRoleId", SqlDbType.Int, 4, ParameterDirection.Input, false, 10, 0, "", DataRowVersion.Proposed, item.AdminRoleMasterID));
                    cmdToExecute.Parameters.Add(new SqlParameter("@iErrorCode", SqlDbType.Int, 4, ParameterDirection.Output, true, 10, 0, "", DataRowVersion.Proposed, _errorCode));
                    ...
                    {
                        AdminRoleCentreRights _item = new AdminRoleCentreRights();
                        if (DBNull.Value.Equals(sqlDataReader["SuperUser"]) == false)
                        {
                            _item.SuperUser = Convert.ToInt32(sqlDataReader["SuperUser"]);
                        }
                        if (DBNull.Value.Equals(sqlDataReader["AcadMgr"]) == false)
                        {
                            _item.AcadMgr = Convert.ToInt32(sqlDataReader["AcadMgr"]);
                        }
                        if (DBNull.Value.Equals(sqlDataReader["EstMgr"]) == false)
                        {
                            _item.EstMgr = Convert.ToInt32(sqlDataReader["EstMgr"]);
                        }
                        if (DBNull.Value.Equals(sqlDataReader["FinMgr"]) == false)
                        {
                            _item.FinMgr = Convert.ToInt32(sqlDataReader["FinMgr"]);
                        }
                        if (DBNull.Value.Equals(sqlDataReader["AdmMgr"]) == false)
                        {
                            _item.AdmMgr = Convert.ToInt32(sqlDataReader["AdmMgr"]);
                        }
                        response.Entity = _item;
                    }
