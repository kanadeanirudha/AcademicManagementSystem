            ...
            try
            {
                ...
                else
                {
                    ...
                    while (sqlDataReader.Read())
                    {
                        ...
                        {
                            item.IsAcadMgrSelf = sqlDataReader["IsAcadMgrStatus"].ToString();
                        }

                        if (DBNull.Value.Equals(sqlDataReader["IsEstMgr"]) == false)
                        {
                            item.IsEstMgrSelf = sqlDataReader["IsEstMgrStatus"].ToString();
                        }

                        if (DBNull.Value.Equals(sqlDataReader["IsFinMgr"]) == false)
                        {
                            item.IsFinMgrSelf = sqlDataReader["IsFinMgrStatus"].ToString();
                        }

                        if (DBNull.Value.Equals(sqlDataReader["IsAdmMgr"]) == false)
                        {
                            item.IsAdmMgrSelf = sqlDataReader["IsAdmMgrStatus"].ToString();
                        }

                        if (DBNull.Value.Equals(sqlDataReader["IsSuperUser"]) == false)
                        {
                            item.IsSuperUserSelf = sqlDataReader["IsSuperUserStatus"].ToString();
                        }
